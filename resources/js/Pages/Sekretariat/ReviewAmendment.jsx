...
export default function ReviewAmendment({ amendments, reviewers = [] }) {
    ...
    const handleDecide = (id, status) => {
        ...
        post(route('sekretariat.amendments.decide', id), {
            data: { status, notes: '' },
    ...
    const handleClassify = (id, type) => {
        post(route('sekretariat.amendments.classify', id), {
            data: { type },
            preserveScroll: true,
        });
    };
    ...
    const handleAssignReviewer = (id) => {
        ...
        post(route('sekretariat.amendments.assignReviewer', id), {
            data: { reviewer_id: selectedReviewer },
...
import { useForm, Head } from '@inertiajs/react';
import Sidebar from '@/Components/Sidebar';
import { FileEdit, CheckCircle, XCircle, Clock, AlertCircle, Filter } from 'lucide-react';
...
export default function ReviewAmendment({ amendments }) {
    const [selectedId, setSelectedId] = useState(null);
    const [filter, setFilter] = useState('all');
    const { post, processing, reset } = useForm({ status: '', notes: '' });
                                    ...
                                    <div className="px-5 pb-5 border-t border-gray-100 pt-4 space-y-4">
                                        ...
                                        {amendment.documents?.length > 0 && (
                                            <div>
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {amendment.documents.map(doc => (
                                                        <a key={doc.id} href={`/storage/${doc.file_path}`} target="_blank"
                                                            className="px-3 py-1.5 bg-blue-50 text-blue-700 rounded-lg text-xs font-semibold hover:bg-blue-100 transition">
                                                            {doc.document_type}
                                                        </a>
                                                    ))}
